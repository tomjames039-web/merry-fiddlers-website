...
async function sendNotificationEmail(lead: LeadData) {
  ...
  try {
    ...
    const eventTypeLabel =
      lead.eventType && lead.eventType in eventTypeLabels
        ? eventTypeLabels[lead.eventType]
        : lead.eventType || 'Event Enquiry';
        ...
        html: `
          <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto;">
            <div style="background: #2d4a4a; color: white; padding: 30px; text-align: center;">
              <h1 style="margin: 0; font-size: 28px;">Thank You, ${lead.fullName?.split(' ')[0] || 'there'}!</h1>
              <p style="margin: 10px 0 0; opacity: 0.9;">We've received your event enquiry</p>
            </div>
            <div style="padding: 30px; background: #f8f6f1;">
              <p style="font-size: 16px; line-height: 1.6; color: #333;">
                Thank you for considering The Merry Fiddlers for your ${lead.eventType?.replace('-', ' ') || 'event'}.
